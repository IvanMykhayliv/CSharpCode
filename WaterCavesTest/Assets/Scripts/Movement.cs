...
public class Movement : MonoBehaviour {
	...
	public class CameraSettings {
        ...
        public bool moveForwardInCamSpace = false;
	}
	[Serializable]
	...
	void FixedUpdate() { //handles physics (always 50 times per second)
		...
		move += vvel;
        //move underwater face camera
        if (cameraSettings.moveForwardInCamSpace) {
            //move = Vector3.Slerp(move, Camera.main.transform.forward, .2f);
            move = Quaternion.FromToRotation(transform.forward, Camera.main.transform.forward) * move;
        }
